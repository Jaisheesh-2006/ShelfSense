...
| **R-14** | **Store_2 unprocessed** — pipeline runs one store; two entrances, 960×1080 | Miss half the data / per-store metrics | Med | Decide D2; API is already per-store, so it's a detection-tier + `store_id`-tagging change. |
